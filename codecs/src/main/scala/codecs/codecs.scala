...
trait EncoderInstances:
  ...
  given [A] (using encoder: Encoder[A]): Encoder[Option[A]] =
    Encoder.fromFunction(optional => optional.map(encoder.encode).getOrElse(Json.Null))
...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromPartialFunction {
      case Json.Arr(items) => items.flatMap(item => decoder.decode(item).toList)
    }
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = ObjectEncoder.field[List[Person]]("people").transform(_.people)

  given Decoder[Contacts] = Decoder.field[List[Person]]("people").transform(Contacts.apply)
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    ...
    val encoderListPerson = summon[Encoder[List[Person]]]
    println(encoderListPerson)
