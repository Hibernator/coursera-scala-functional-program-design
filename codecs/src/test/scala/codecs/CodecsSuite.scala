...
class CodecsSuite
  ...
  test("it is possible to decode an object") {
    val obj = Json.Obj(Seq("boolField" -> Json.Bool(true), "stringField" -> Json.Str("someString")).toMap)
    val decoder: Decoder[(Boolean, String)] =
      val boolField = Decoder.field[Boolean]("boolField")
      val strField = Decoder.field[String]("stringField")
      boolField.zip(strField)

    assert(decoder.decode(obj) == Some(true, "someString"))
  }

  test("it is possible to encode an optional value") {
    val optionalInt = Some(42)
    val encoder = summon[Encoder[Option[Int]]]
    assert(encoder.encode(optionalInt) == Json.Num(42))
