...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(
    ...
    LazyList(
      (b.left, Move.Left :: history),
      (b.right, Move.Right :: history),
      (b.up, Move.Up :: history),
      (b.down, Move.Down :: history)
    ).filter { (block, _) => block.isLegal }
  ...
  def from(
    ...
    val maxHistoryLength =
      Option.when(initial.nonEmpty)(initial.map(_._2).maxBy(_.size).size)
    val nextStates = for
      (block, history) <- initial if maxHistoryLength.forall(_ == history.size)
      newState <- newNeighborsOnly(
        neighborsWithHistory(block, history),
        explored
      )
    yield newState

    initial #::: from(nextStates, explored ++ nextStates.map(_._1))

  /** The lazy list of all paths that begin at the starting block.
    */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(List((startBlock, List[Move]())).to(LazyList), Set[Block]())
  ...
  lazy val solution: List[Move] =
    pathsToGoal.headOption.map(_._2.reverse).getOrElse(List[Move]())
